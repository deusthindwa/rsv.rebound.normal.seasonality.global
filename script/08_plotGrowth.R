...
A <-
  ...
  labs(x = "", y = "RSV growth rate in 2021/22", title = "") +
...
B <-
  ...
  scale_y_continuous(breaks = seq(0, 1.2, 0.2), limits = c(0, 1.3)) +
  theme_bw(base_size = 14, base_family = 'Lato') +
  labs(x = "", y = "RSV growth rate in 2022/23", title ="") +
  scale_shape_manual(values = c(18, 4)) +
  theme(legend.position = "none", legend.title = element_blank(), axis.text.x = element_blank(), strip.text.x = element_text(size = 16)) + 
  theme(panel.border = element_rect(colour = "black", fill = NA, size = 2)) +
  guides(shape = FALSE)
...
D <-
  ...
  scale_y_continuous(breaks = seq(0, 1.2, 0.2), limits = c(0, 1.3)) +
  theme_bw(base_size = 14, base_family = 'Lato') +
  labs(x = "PreCOVID (17-19) mean growth rate", y = "RSV growth rate in 2022/23", title ="") +
...
ggsave(here("output", "fig4_growthMain.png"),
       plot = (A/B/C/D),
...
E <-
  ...
  geom_line(data = rsv_growth1, aes(x = seqwk, y = deriv/0.01), size = 0.8, color = "darkred") + # differentiated fitted curve
  scale_y_continuous("Repoted and fitted weekly RSV cases", sec.axis = sec_axis(~. *0.01, name = "Estimated growth rate")) + 
  geom_point(data = rsv_growth1, aes(x = seqwk, y = pks/0.01), color = "darkred", shape = 4, size = 1,  stroke = 2) + #timing of peak and peak value
...
ggsave(here("output", "fig4_growthSupp.png"),
       plot = (E),
