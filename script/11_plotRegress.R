...
DSonset1Mod %>%
  ...
  dplyr::mutate(year = "Onset in 2021 from July 2020", model = "Time to RSV Onset \n (Survival Model)"),

DSonset2Mod %>%
  dplyr::select(term, estimate, p.value, conf.low, conf.high) %>%
  dplyr::rename("p_value" = "p.value", "lower_ci" = "conf.low", "upper_ci" = "conf.high") %>%
  dplyr::mutate(year = "Onset in 2022 from Onset in 2021", model = "Time to RSV Onset \n (Survival Model)"),

DSpeak1Mod %>%
  dplyr::select(term, estimate, p.value, conf.low, conf.high) %>%
  dplyr::rename("p_value" = "p.value", "lower_ci" = "conf.low", "upper_ci" = "conf.high") %>%
  dplyr::mutate(year = "Peak in 2021 from July 2020", model = "Time to RSV Peak \n (Survival Model)"),

DSpeak2Mod %>%
  dplyr::select(term, estimate, p.value, conf.low, conf.high) %>%
  dplyr::rename("p_value" = "p.value", "lower_ci" = "conf.low", "upper_ci" = "conf.high") %>%
  dplyr::mutate(year = "Peak in 2022 from Peak in 2021", model = "Time to RSV Peak \n (Survival Model)")) %>%
...
X2 <-            
  ...
  DSgrowth1Mod %>%
    ...
    dplyr::mutate(year = "Growth rate in 2021", model = "RSV Growth Rate \n (Linear Model)"),
  
  DSgrowth2Mod %>%
    dplyr::select(term, estimate, p_value, lower_ci, upper_ci) %>%
    dplyr::mutate(year = "Growth rate in 2022", model = "RSV Growth Rate \n (Linear Model)"),
...
A <-
  DSregress %>%
  ggplot(aes(y = reorder(term, year), x = estimate, color = year, width = widthx)) +
  geom_point(shape = 16, size = 4, stroke = 2, position = position_dodge(width = 0.5)) + 
  geom_errorbarh(aes(xmin = lower_ci, xmax = upper_ci), height = 0, size = 1.5, position = position_dodge(width = 0.5)) +
  geom_vline(xintercept = 0, color = "black", linetype = "dashed", cex = 0.6, alpha = 0.8) +
  labs(title = "", x = "Log-scale estimates from eight multivariate regression models with 95%CI", y = "") + 
