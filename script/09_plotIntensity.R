...
A <-
  ...
  labs(x = "", y = "RSV intensity in 2021/22", title = "") +
...
D <-
  ...
  scale_y_continuous(breaks = seq(0, 10, 1), limits = c(0, 10)) +
  theme_bw(base_size = 14, base_family = 'Lato') +
  labs(x = "PreCOVID (17-19) mean intensity", y = "RSV intensity in 2022/23", title ="") +
...
ggsave(here("output", "fig5_intensityMain.png"),
       plot = (A/B/C/D),
...
E <-
  ...
  geom_line(data = rsv_intens1, aes(x = seqwk, y = deriv/0.002), size = 0.8, color = "darkred") + # differentiated fitted curve
  scale_y_continuous("Repoted and fitted weekly RSV cases", sec.axis = sec_axis(~. *0.002, name = "Estimated growth rate")) + 
...
ggsave(here("output", "fig5_intensitySupp.png"),
       plot = (E),
