...
ggsave(here("output", "fig3_corHemisphere.png"),
       plot = ((A1 + labs(title = "RSV ONSET TIMING"))/(A2 + labs(x = "PreCOVID (2017-19) mean estimate"))),
       width = 14, height = 9, unit = "in", dpi = 300)
...
#===================================================================================
# CREATE DATASET FOR TIME TO ONSET WEEK DURING SECOND WAVE FROM COVID-19 SUPPRESSION
#===================================================================================

#time to first onset in 2022 from 2020 COVID-19 suppression in July
DSonset3 <-
  rsv_onset %>%
  dplyr::select(country, covper, epiwk, clim_zone, hemi) %>%
  dplyr::left_join(climate) %>%
...
#==========================================================================
# CREATE DATASET FOR TIME TO PEAK WEEK FIRST WAVE FROM COVID-19 SUPPRESSION
#==========================================================================
...
#==========================================================================
# CREATE DATASET FOR TIME TO PEAK WEEK SECOND WAVE FROM COVID-19 SUPPRESSION
#==========================================================================
...
#ONSET DURING FIRST WAVE FROM COVID-19 SUPPRESSION
...
#ONSET DURING SECOND WAVE FROM FIRST WAVE
#standardize all continuous variables
DSonset2 <- 
  DSonset2 %>% 
  mutate(strin_indx2 = (strin_indx - mean(strin_indx))/sd(strin_indx),
...
#ONSET DURING SECOND WAVE FROM COVID-19 SUPPRESSION
#standardize all continuous variables
DSonset3 <- 
  DSonset3 %>% 
  mutate(strin_indx2 = (strin_indx - mean(strin_indx))/sd(strin_indx),
...
X <- list()
for (i in c("hemi2", "clim_zone2", "strin_indx2", "pop_dens2", "med_age2", "out_seas212", "intens20212")) {
  X[[i]] <- print(tidy(coxph(as.formula(paste0("Surv(time, event) ~", i)), data = DSonset3, control = coxph.control(iter.max = 1000)), exponentiate = TRUE, conf.int = TRUE, conf.level = 0.95))}
XX2 <- dplyr::bind_rows(X) %>% dplyr::mutate(ds = "Onset timing 2022") %>% dplyr::filter(term != "(Intercept)") %>% dplyr::select(term, estimate, conf.low, conf.high, p.value, ds)

#================================================================

#PEAK DURING FIRST WAVE FROM COVID-19 SUPPRESSION
#standardize all continuous variables
DSpeak1 <- 
  DSpeak1 %>% 
  mutate(strin_indx2 = (strin_indx - mean(strin_indx))/sd(strin_indx),
...
#PEAK DURING SECOND WAVE FROM FIRST WAVE
...
#PEAK DURING SECOND WAVE FROM COVID-19 SUPPRESSION
#standardise all continuous variables
DSpeak3 <- 
  DSpeak3 %>% 
  mutate(strin_indx2 = (strin_indx - mean(strin_indx))/sd(strin_indx),
...
X <- list()
for (i in c("hemi2", "clim_zone2", "strin_indx2", "pop_dens2", "med_age2", "out_seas212", "intens20212")) {
  X[[i]] <- print(tidy(coxph(as.formula(paste0("Surv(time, event) ~", i)), data = DSpeak3, control = coxph.control(iter.max = 1000)), exponentiate = TRUE, conf.int = TRUE, conf.level = 0.95))}
XX4 <- dplyr::bind_rows(X) %>% dplyr::mutate(ds = "Peak timing 2022") %>% dplyr::filter(term != "(Intercept)") %>% dplyr::select(term, estimate, conf.low, conf.high, p.value, ds)
