...
#run the GAM models where knots are selected via Random Effects Maximum Likelihood (REML)
for (i in names(X)){
...
#run simulations on an outbreak GAM to estimate time series outbreak outcomes 
for (i in names(X)){
...
for (i in names(X)){
  ...
  tsDS = data.frame(fitcases = data$cases) %>% 
    mutate(seqwk = seq.int(from = 1, by = 1, length.out = n()))
  
  deriv1.pred = data.frame(deriv = diff(tsDS$fitcases)/diff(tsDS$seqwk)) %>%
    mutate(seqwk = seq.int(from = 1, by = 1, length.out = n()))
  
  deriv2.pred = data.frame(deriv2 = diff(deriv1.pred$deriv)/diff(deriv1.pred$seqwk)) %>%
    mutate(seqwk = seq.int(from = 1, by = 1, length.out = n()))
  
  rsv_onset2 = data.frame(gsignal::findpeaks(deriv2.pred$deriv2, MinPeakHeight = 0.01, MinPeakDistance = 1, MinPeakWidth = 1, MaxPeakWidth = Inf, DoubleSided = TRUE))
    })(.)) %>%
    ungroup()  
  }
...
deriv1.pred <- bind_rows(deriv.pred, .id = "country")
deriv2.pred <- bind_rows(deriv2.pred, .id = "country")
rsv_onset2 <- bind_rows(onset.samples, .id = "country") %>% dplyr::filter(pks > 0)
